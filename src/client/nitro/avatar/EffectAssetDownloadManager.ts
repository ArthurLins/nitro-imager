...
export class EffectAssetDownloadManager extends EventDispatcher
{
	...
	constructor(configs:ConfigurationManager, assets: IAssetManager, structure: AvatarStructure)
	{
	    ...
	    this._missingMandatoryLibs  = this._configurationManager.getValue<string[]>('avatar.mandatory.effect.libraries');
	...
	private loadEffectMap(): void
	{
	    ...
	    try
	    {
	        request.open('GET', this._configurationManager.getValue<string>('avatar.effectmap.url'));
	...
	private processEffectMap(data: any): void
	{
	    ...
	    for(const effect of data)
	    {
	        ...
	        const downloadLibrary = new EffectAssetDownloadLibrary(lib, revision, this._assets, this._configurationManager.getValue<string>('avatar.asset.effect.url'));
